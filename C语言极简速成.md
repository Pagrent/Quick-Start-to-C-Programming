...
> 
> stdio.h的意思是“标准输入输出头文件”，这样我们才能使用`printf`和`scanf`等函数。
> 
> ```c
> int main() {
> 
...
> 
> 另外，我们有时候会见到 `//` `/* */` ，这是c语言的注释。注释是给人看的，不影响程序的运行。
> `//`是单行注释，仅覆盖从`//`到这一行结尾的所有内容
> `/* */`是多行注释（注释块），注释内容包含从`/*`开始，直到到`*/`之前的所有内容。
...
就像“同时满足多个条件”或“满足至少一个条件”：

- **`&&`（与）**：两个条件都要成立（类似于“并且”）；
  
- **`||`（或）**：两个条件至少有一个成立（类似于“或者”）；
  
- **`!`（非）**：取反，成立变不成立、不成立变成立。
  

#### 示例1（判断年龄是否在 18~60 岁之间）
...
int main() {
    ...
    // 使用 && 判断“18 ≤ age ≤ 60”
    if (age >= 18 && age <= 60) {
        printf("年龄 %d 在 18~60 之间\n", age);
    } else {
        printf("年龄 %d 不在 18~60 之间\n", age);
    }

    // 使用 || 判断“age < 18 或 age > 60”
    if (age < 18 || age > 60) {
        printf("年龄 %d 不在工作年龄范围内\n", age);
    } else {
        printf("年龄 %d 在工作年龄范围内\n", age);
    }
...
> ~~（点名批评首师大某c程序设计教师）~~：
> 
> > 如果某一行不以分号结尾（例如大括号内），下一行一定要右移4个空格
> > 
> > 运算符（+ - * / & % ! = < > <= >= ==）两边要加空格
> > 
...
内存里本来是一个个独立的房间，数组就是向计算机说：“给我开一排连续的、同户型的房间，从这个门牌号开始，连续N个！”
比如`int scores[5]; `就是“给我5个连续的int房间（共20字节），第一个房间的昵称叫scores”。

- 数组名（scores）其实代表“这排房间的第一个房间的门牌号”
- 用下标访问：`scores[0]` = 第一间，`scores[1]` = 第二间……`scores[4]` = 第五间
- 下标从0开始（C语言的传统，记住就好）
...
```c
int nums[5] = {10, 20, 30, 40, 50};          // 完整初始化
int nums2[] = {1, 2, 3};                     // 自动推断长度为3
int nums3[5] = {0};                          // 所有房间都放0
```

如果不进行初始化，数组内会存在一些随机的数

### 示例3：字符数组与字符串

```c
char name[10] = "ZhangSan";  // 实际占9字节，最后自动加'\0'
printf("姓名：%s\n", name);   // %s会一直打印直到遇到'\0'
```

### 关键注意点

- 下标千万别越界（`scores[5] = 100;` 会改到别人家的房间，程序可能崩溃）
- 数组名在大部分情况下是指向第一个元素的指针（后面指针章节会讲指针是什么）
- 数组大小在定义时必须是常量（一开始就要说清楚需要多少房间）
...
## 6. 宏定义与typedef：给C语言“起别名、做快捷指令”

这两个是C语言的“懒人神器”——让代码更短、更好懂，是大型程序中简化代码的核心手段。

### 6.1 宏定义（#define）：快捷指令/常量别名
